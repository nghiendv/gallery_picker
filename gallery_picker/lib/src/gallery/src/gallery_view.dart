...
import 'package:gallery_picker/src/camera/camera_view.dart';
...
class GalleryView extends StatefulWidget {
  ...
  ///
  static Future<List<GallaryEntity>?> pick(BuildContext context) async {
    return Navigator.of(context).push<List<GallaryEntity>>(
...
class GalleryViewField extends StatefulWidget {
  ...
  ///
  final void Function(List<GallaryEntity> entities)? onSubmitted;
...
class GalleryController extends ValueNotifier<GalleryValue> {
  ...
  Completer<List<GallaryEntity>> _completer = Completer<List<GallaryEntity>>();
  ...
  void Function(GallaryEntity entity, bool removed)? _onChanged;

  //  Gallery picker on submitted event callback handler
  void Function(List<GallaryEntity> entities)? _onSubmitted;
  ...
  List<GallaryEntity> _cachedInitList = [];
  ...
  void clearSelection([List<GallaryEntity>? list]) {
    if (list == null) {
      _onSubmitted?.call([]);
  ...
  void _select(GallaryEntity entity, BuildContext context) {
  ...
  Future<GallaryEntity?> openCamera(BuildContext context) async {
    _accessCamera = true;
    GallaryEntity? entity;

    final route = SlideTransitionPageRoute<GallaryEntity>(
  ...
  Future<void> _openGallery(
    void Function(GallaryEntity entity, bool removed)? onChanged,
    final void Function(List<GallaryEntity> entities)? onSubmitted,
  ...
  /// Pick assets
  Future<List<GallaryEntity>> pick(
    BuildContext context, {
    List<GallaryEntity>? selectedEntities,
  }) async {
    ...
    _completer = Completer<List<GallaryEntity>>();
    galleryState.value = GalleryState.show;

    if (_wrapperKey.currentState == null) {
      _fullScreenMode = true;
      final route = SlideTransitionPageRoute<List<GallaryEntity>>(
